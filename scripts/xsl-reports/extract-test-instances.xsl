...
<xsl:for-each select="oval-res:results/oval-res:system/oval-res:tests/oval-res:test[@test_id=$testid]">
<xsl:for-each select="oval-res:tested_item">
<xsl:value-of select="$testid" />,<xsl:value-of select="@item_id" />,<xsl:value-of select="@result" /><xsl:text>
</xsl:text>
</xsl:for-each>
</xsl:for-each>
</xsl:template>
